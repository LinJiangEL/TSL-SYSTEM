...
## Special-Win32-Release
### 1.功能概述
- 包含系统原设计指令组的少部分功能（用户管理指令组user无法对用户数据库执行有效[permanent]操作）。
- 包含Passwd, Mathematics, Translate的大部分功能（一些功能例如Translate的字典功能由于UPX而被迫移除）。
### 2.总而言之
> 目前只有Windows平台的二进制可执行文件，由于UPX的压缩和不保留数据机制，对文件的修改不会永久生效，所以一切对程序数据的操作都是不被建议的。
### 3.版权声明
> 版权这个问题我不想多说，如果要转载请注明出处，否则别怪我们一整个实验室的人招呼你，我们也没那闲工夫管版权这个问题。
